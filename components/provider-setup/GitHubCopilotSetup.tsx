...
export function GitHubCopilotSetup() {
	const router = useRouter();
	const { providers, setActiveProvider } = useAIProvider();
	const provider = providers.find((p) => p.id === "github-copilot");

	const [isConnecting, setIsConnecting] = useState(false);
	const [copied, setCopied] = useState(false);

	const error = useCell("aiProviders", "github-copilot", "error") as
		| string
		| undefined;
	const status = useCell("aiProviders", "github-copilot", "status") as
		| string
		| undefined;
	const modelCard = useCell("aiProviders", "github-copilot", "modelCard") as
		| string
		| undefined;
	...
	const handleConnect = async () => {
		...
		try {
			...
			console.error("[GitHubCopilotSetup] OAuth error:", err);
		} finally {
				...
				>
					...
					>
						...
						>
							...
							Your conversations are sent to GitHub's servers.
						</Text>
					</View>
					...
					{status === "ready" ? (
						<View style={{ gap: 16, width: "100%", paddingBottom: 48 }}>
							...
							>
								You're connected to GitHub Copilot
							</Text>
